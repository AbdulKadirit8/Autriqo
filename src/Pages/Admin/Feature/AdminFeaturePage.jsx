    ...
    <div className='container-fluid my-3'>
      <div className="row">
        ...
        <div className={`${showSlider ? 'col-12' : 'col-md-9'}  fadeInRight animated`} data-animation="fadeInRight" data-delay="0.1s" style={{ animationDelay: "0.1s" }}>
          <h5 className='bg-primary p-2 fs-4 text-light text-center rounded-top'><i className={`bi ${showSlider ? 'bi-list' : 'bi-x-circle'} float-start fs-3 d-none d-md-inline`} onClick={() => dispatch(setShowSlider(!showSlider))}></i>Feature <Link to="/admin/feature/create"><i className='bi bi-plus text-light float-end fs-3'></i></Link></h5>
              ...
              <tbody>
                {data.map(item => {
                  return <tr key={item.id}>
                    ...
                    <td><button className='btn btn-primary'><Link to={`/admin/feature/update/${item.id}`}><i className='bi bi-pencil text-light'></i></Link></button></td>
